...
/**
 * @author Darly/张宇辉/2018/4/3 14:52
 * @version 1.0/com.darly.darlyview.games.tile.view
 */
public class TileLevel extends TileImage {
    ...
    public TileLevel(Context context, Point point) {
        super(context);
        this.mX = point.x;
        this.mY = point.y;
    }

    public TileLevel(Context context, int drawable, Point point) {
    ...
    public boolean getCollision(TileUnit tileUnit) {
        return (tileUnit.getRect().intersects(this.mX, this.mY, (this.mX + mWidth), (this.mY + mHeight)));
    }
    ...
    public boolean isCollisionTile() {
        return ((this.mType != TileLevel.TYPE_EMPTY) && this.mVisible);
    }

    public boolean isBlockerTile() {
        if (this.mType == TileLevel.TYPE_EMPTY) {
            return false;
        }
        return true;
    }
